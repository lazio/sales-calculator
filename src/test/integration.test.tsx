...
describe('Integration Tests', () => {
  ...
  describe('Module Toggle and Timeline Integration', () => {
    const mockRates: RateConfig[] = [
      { role: 'UI Designer', monthlyRate: 4000 },
    ];

    it('should highlight modules that won\'t fit in timeline', () => {
      ...
      // Module is enabled but not in timeline (excluded by compression)
      render(
        <ModuleList
          modules={modules}
          onToggle={vi.fn()}
          rates={mockRates}
          modulesInTimeline={[]} // Module excluded from timeline
          currency="$"
        />
      );

      // Should show warning badge
      expect(screen.getByText(/won't fit in timeline/i)).toBeInTheDocument();
    });

    it('should not show warning for modules that fit in timeline', () => {
      const modules: ProjectModule[] = [
        {
          id: 'module-1',
          name: 'Small Module',
      ...
      // Module is in timeline
      render(
        <ModuleList
          modules={modules}
          onToggle={vi.fn()}
          rates={mockRates}
          modulesInTimeline={['module-1']} // Module included
          currency="$"
        />
      );

      // Should not show warning
      expect(screen.queryByText(/won't fit in timeline/i)).not.toBeInTheDocument();
    });
  });
